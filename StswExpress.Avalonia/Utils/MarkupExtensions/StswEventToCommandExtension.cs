...
public class StswEventToCommandExtension : MarkupExtension
{
    ...
    public override object? ProvideValue(IServiceProvider serviceProvider)
    {
        ...
        if (pvt.TargetObject is not AvaloniaObject d)
            return null;
    ...
    private Delegate? CreateHandler(Type? eventHandlerType, AvaloniaObject? targetObject)
    {
        if (targetObject == null || eventHandlerType == null)
            return null;

        if (eventHandlerType == typeof(EventHandler<KeyEventArgs>))
        {
            return new EventHandler<KeyEventArgs>((sender, e) =>
            {
    ...
    private Delegate? CreateHandlerFromMethodInfo(MethodInfo methodInfo, AvaloniaObject targetObject)
    {
    ...
    private void ExecuteCommand(AvaloniaObject targetObject, EventArgs e)
    {
    ...
    private object? ResolveParameter(AvaloniaObject target, EventArgs e)
    {
    ...
    private static T? EvaluateBinding<T>(AvaloniaObject target, BindingBase bindingBase)
    {
        if (bindingBase == null)
            return default;

        var disposable = target.Bind(TempValueProperty, bindingBase);
    ...
    private static object CreateDefaultHandlerViaExpression(Type delegateType, AvaloniaObject targetObj, StswEventToCommandExtension ext)
    {
    ...
    /// </summary>
    private static readonly AvaloniaProperty TempValueProperty
