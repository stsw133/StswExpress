            ...
            switch (param)
            {
                ...
                default:
                    multiBinding.Bindings.Add(new Binding
                    {
                        ...
                        Path = nameof(ObjectValueProvider.Value)
                    });
                    break;
            }
        }
    ...
    public class InvokeMethodConverter : IMultiValueConverter
    {
        ...
        public object? Convert(IList<object?> values, Type targetType, object? parameter, CultureInfo culture)
        {
            ...
            var method = type.GetMethods(BindingFlags.Public | BindingFlags.Static)
                .FirstOrDefault(m =>
                    m.Name == methodName &&
                    m.GetParameters().Length == values.Count);

            return method == null
                ? throw new MissingMethodException($"Method '{methodName}' not found with {values.Count} parameter(s).")
