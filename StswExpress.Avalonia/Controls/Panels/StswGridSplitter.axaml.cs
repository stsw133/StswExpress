...
public class StswGridSplitter : GridSplitter
{
    ...
    /// <summary>
    /// Handles the AttachedToVisualTree event for the splitter. It retrieves the parent grid and the index of the splitter within the grid.
    /// </summary>
    ...
    /// <summary>
    /// Handles the DoubleTapped event for the splitter. It resets the adjacent column or row to its original length.
    /// </summary>
    /// <param name="sender">The sender of the event, typically the splitter itself.</param>
    /// <param name="e">The event arguments containing the routed event data.</param>
    ...
    private void InitOriginalLength()
    {
        ...
        {
            var col = Grid.GetColumn(this);
            CaptureColumnLength(col - 1);
            CaptureColumnLength(col + 1);
        }
        else
        {
            var row = Grid.GetRow(this);
            CaptureRowLength(row - 1);
            CaptureRowLength(row + 1);
        }
    }
