...
public partial class StswMessageDialogContext : ControlsContext
{
    public override void SetDefaults()
    {
        ...
        Buttons = (StswDialogButtons?)ThisControlSetters.FirstOrDefault(x => x.Property.Name.Equals(nameof(Buttons)))?.Value ?? default;
        ...
        Image = (StswDialogImage?)ThisControlSetters.FirstOrDefault(x => x.Property.Name.Equals(nameof(Image)))?.Value ?? default;
    }
    ...
    [StswCommand] async Task OpenMessageDialog()
    {
        var result = await StswMessageDialog.Show(
            ...
            new StswMessageDialogShowOptions { MailAddress = MailAddress, SaveLog = true });
        
        DialogResult = result?.ToString();
    }

    [StswObservableProperty] StswDialogButtons _buttons;
    [StswObservableProperty] bool _closeOnBackdropClick;
    [StswObservableProperty] string? _dialogResult;
    [StswObservableProperty] StswDialogImage _image;
    [StswObservableProperty] string? _mailAddress;
